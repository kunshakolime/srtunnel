...
if [[ -f "$REPO_ROOT/configs/config.template.yaml" && -f "$REPO_ROOT/configs/srtdash.nginx" ]]; then
    BOT_DIR="$REPO_ROOT"
elif [[ -f "$SCRIPT_DIR/configs/config.template.yaml" && -f "$SCRIPT_DIR/configs/srtdash.nginx" ]]; then
    BOT_DIR="$SCRIPT_DIR"
fi
cd "$BOT_DIR"
...
# ── HTTP vhost (srtdash) ────────────────────────────────────────────────────
# configs/srtdash.nginx contains ${DOMAIN} and ${BOT_DIR} — replace with hardwired values
if [[ -f configs/srtdash.nginx ]]; then
    sed -e "s|\${DOMAIN}|example.com|g" -e "s|\${BOT_DIR}|$BOT_DIR|g" configs/srtdash.nginx > /etc/nginx/sites-available/srtdash
    ln -sf /etc/nginx/sites-available/srtdash /etc/nginx/sites-enabled/srtdash
else
    echo "WARNING: configs/srtdash.nginx not found in $BOT_DIR — skipping vhost" >&2
fi
