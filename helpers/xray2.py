...
XUI_USER = _xray_cfg.get("username", "")
XUI_PASS = _xray_cfg.get("password", "")
...
class XUIClient:
    ...
    def login(self, username: str, password: str, two_factor_code: str = "") -> Dict:
        payload = {"username": username, "password": password, "twoFactorCode": two_factor_code}
        logger.info(f"Attempting login for user: {username}")
        resp = self.session.post(self._url("login/"), data=payload, allow_redirects=True)
        resp.raise_for_status()
        data = resp.json()
        if not data.get("success"):
            raise RuntimeError(f"Login failed: {data}")
        logger.info("Login successful")
        return {"success": True, "data": data, "cookies": self.session.cookies.get_dict()}
    ...
    def add_inbound(self, inbound_config: Dict) -> Dict:
        ...
        resp = self.session.post(self._url("panel/api/inbounds/add"), json=payload, headers={"Content-Type": "application/json", "Accept": "application/json"})
        resp.raise_for_status()
        return resp.json()

    def delete_inbound(self, inbound_id: int) -> Dict:
        logger.info(f"Deleting inbound {inbound_id}")
        resp = self.session.post(self._url(f"panel/api/inbounds/del/{inbound_id}"), headers={"Accept": "application/json"})
        resp.raise_for_status()
        return resp.json()
    ...
    def remove_user(self, inbound_id: int, client_uuid: str) -> Dict:
        ...
        try:
            resp = self.session.post(self._url(f"panel/api/inbounds/{inbound_id}/delClient/{client_uuid}"), headers={"Accept": "application/json"})
            resp.raise_for_status()
        except requests.exceptions.HTTPError as e:
